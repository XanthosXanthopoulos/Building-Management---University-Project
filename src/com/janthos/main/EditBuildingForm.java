...
public class EditBuildingForm extends JDialog
{
	...
	public EditBuildingForm(JFrame parent, Company company, Building building, DefaultTableModel buildingTableModel, int row)
	{
		setTitle("Edit/Show Building");
		setModal(true);
		...
		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) 
		    {
				...
				if (result == JOptionPane.YES_OPTION)
				{
					...
					if ((!company.checkBuildingCodeAvailability(code) && !code.equals(originalCode)) || code.equals(""))
					{
					...
					buildingTableModel.setValueAt(code, row, 0);
					buildingTableModel.setValueAt(description, row, 1);
				}
				thisFrame.dispose();
		    }
		});
		...
		zoneValueTextField.setColumns(15);
		zoneValueTextField.setValue(0);
		//Formatted Text Fields - End
		
		//Set initial values - Begin
		originalCode = building.getCode();
		codeTextField.setText(building.getCode());
		descriptionTextField.setText(building.getDescription());
		addressTextField.setText(building.getAddress());
		areaTextField.setValue(building.getArea());
		zoneValueTextField.setValue(building.getZoneValue());
